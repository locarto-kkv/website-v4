DROP VIEW IF EXISTS weekly_vendor_summary;

CREATE VIEW public.weekly_vendor_summary AS
WITH vendor_orders AS (
  ...
  LEFT JOIN public.orders o
         ON o.id = oi.order_id
  WHERE o.id IS NOT NULL  -- only include vendor-weeks with real orders
)
SELECT
    vo.vendor_id,
    vo.vendor_name,
    ...
    ARRAY_AGG(DISTINCT vo.consumer_id) 
        FILTER (WHERE vo.consumer_id IS NOT NULL) AS consumer_ids,

    COUNT(DISTINCT vo.consumer_id) AS consumers_count,
    COUNT(DISTINCT vo.product_id) AS products_count,

    /* --- distinct orders only --- */
    COUNT(DISTINCT vo.order_id) AS orders_count,

    /* --- dedup total amount to prevent inflated sums --- */
    (
      SELECT SUM(amount)
      FROM (
        ...
        WHERE o2.vendor_id = vo.vendor_id
          AND o2.order_week = vo.order_week
      ) x
    ) AS total_amount,
    ...
    /* --- category counts (same vendor-week) --- */
    (
...
GROUP BY vo.vendor_id, vo.vendor_name, vo.order_week
ORDER BY vo.vendor_id, vo.order_week;
