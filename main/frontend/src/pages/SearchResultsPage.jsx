...
const SearchResultsPage = () => {
  ...
  useEffect(() => {
    const fetchResults = async () => {
      ...
      setLoading(true);
      try {
        const data = await ConsumerSearchService.getSearchResults(queryParam);
  ...
  const showAuthToast = () => {
    toast((t) => (
      <div className="flex flex-col gap-2 items-start">
        <span className="font-medium text-gray-800">Not signed in as customer</span>
        <button
          onClick={() => {
            toast.dismiss(t.id);
            navigate("/consumer/login", { state: { isSignup: true, from: window.location.pathname + window.location.search } });
          }}
    ...
    ), { duration: 5000, icon: "🔒", style: { background: "#fff", color: "#333", border: "1px solid #e5e7eb" } });
  };
  ...
  const toggleWishlist = async (e, product) => {
    ...
    try {
      if (isInWishlist) {
        ...
        useConsumerDataStore.setState((state) => ({ ...state, lists: { ...newList } }));
  ...
  const handleCartChange = async (e, product, delta) => {
    ...
    if (vendorInCart && vendorId !== vendorInCart && !cartItem) {
      toast.error("You can only add items from one brand at a time. Please clear your cart first.");
      return;
    }
  ...
  const processedProducts = useMemo(() => {
    ...
    // Rating Filter
    if (minRating > 0) {
      filtered = filtered.filter((p) => (Number(p.avg_review) || 0) >= minRating);
    }
                  ...
                  <div className="space-y-2">
                    {["all", "Personal Care", "Accessories"].map((cat) => (
                      <label key={cat} className="flex items-center gap-2 cursor-pointer group">
                        ...
                        />
                        <span className={`text-sm ${selectedCategory === cat ? 'text-gray-900 font-semibold' : 'text-gray-600 group-hover:text-gray-800'}`}>
                    ...
                    {[
                      ...
                      <label key={opt.val} className="flex items-center gap-2 cursor-pointer group">
                        <input
                          ...
                          className="text-orange-500 focus:ring-orange-500 border-gray-300"
                        />
                        <span className={`text-sm ${priceRange === opt.val ? 'text-gray-900 font-semibold' : 'text-gray-600 group-hover:text-gray-800'}`}>
                          {opt.label}
                        </span>
                      </label>
                    ))}
                  ...
                  <div className="space-y-2">
                    {[4, 3, 2].map((r) => (
                      <label key={r} className="flex items-center gap-2 cursor-pointer group">
                        <input
                          ...
                          className="text-orange-500 focus:ring-orange-500 border-gray-300"
                        />
                        <span className={`text-sm flex items-center gap-1 ${minRating === r ? 'text-gray-900 font-semibold' : 'text-gray-600 group-hover:text-gray-800'}`}>
                    ...
                    <label className="flex items-center gap-2 cursor-pointer group">
                        <input
                          ...
                          className="text-orange-500 focus:ring-orange-500 border-gray-300"
                        />
                        <span className={`text-sm ${minRating === 0 ? 'text-gray-900 font-semibold' : 'text-gray-600 group-hover:text-gray-800'}`}>
                          Any Rating
                        </span>
                      </label>
                  </div>
                ...
                <div className="flex gap-2 bg-white p-1 rounded-xl shadow-sm border border-gray-100">
                  <button
                    ...
                    className={`px-4 py-2 rounded-lg text-sm font-medium transition-all ${
                      activeTab === "all" ? "bg-orange-50 text-orange-700" : "text-gray-600 hover:text-gray-900"
                    }`}
                  >
                    All Results
                  </button>
                  <button
                    ...
                    className={`px-4 py-2 rounded-lg text-sm font-medium transition-all ${
                      activeTab === "products" ? "bg-orange-50 text-orange-700" : "text-gray-600 hover:text-gray-900"
                    }`}
                  >
                  ...
                  <button
                    ...
                    className={`px-4 py-2 rounded-lg text-sm font-medium transition-all ${
                      activeTab === "brands" ? "bg-orange-50 text-orange-700" : "text-gray-600 hover:text-gray-900"
                    }`}
                  >
              ...
              <div className="text-center py-20 bg-white rounded-3xl shadow-sm border border-gray-100">
                <div className="max-w-md mx-auto">
                  ...
                  <p className="text-gray-600 mb-8 leading-relaxed">
                    We couldn't find matches for <span className="font-semibold text-gray-900">"{queryParam}"</span>.
                  </p>
                  <Link
                    to="/map"
                            ...
                            <div className="relative aspect-[4/3] bg-gray-100 overflow-hidden">
                              ...
                              <button
                                ...
                                className={`absolute top-3 right-3 p-2 rounded-full shadow-md transition-all z-10 ${
                                  isInWishlist ? 'bg-white text-red-500' : 'bg-black/40 text-white hover:bg-white hover:text-red-500'
                                }`}
                              >
                                <i className={`${isInWishlist ? 'fas' : 'far'} fa-heart text-lg`}></i>
                              </button>
                            ...
                            <div className="p-4 flex flex-col flex-grow">
                              ...
                              <div className="pt-3 border-t border-gray-100 flex items-center justify-between">
                                <div className="flex items-center gap-1 text-yellow-400 text-xs">
                                  ...
                                  <span className="font-bold text-gray-700 text-sm">{product.avg_review || "0.0"}</span>
                                </div>
                                ...
                                {quantity > 0 ? (
                                  <div className="flex items-center gap-2 bg-gray-100 rounded-lg p-1">
                                    <button
                                      onClick={(e) => handleCartChange(e, product, -1)}
