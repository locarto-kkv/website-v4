...
const VendorMilestones = () => {
    ...
    const { currentRate: currentCommissionRate, nextMilestone: nextCommissionMilestone } = getCurrentCommissionDetails(currentTotalRevenue); //
    ...
    const allDisplayMilestones = [ // Renamed variable //
    ...
    if (nextCommissionMilestone) { //
        ...
        const milestoneRange = nextCommissionMilestone.targetRevenue - previousMilestoneRevenue; //
        const progressInRange = currentTotalRevenue - previousMilestoneRevenue; //

        commissionProgress = Math.min(100, Math.max(0, (progressInRange / milestoneRange) * 100)); //
        revenueToNextMilestone = Math.max(0, nextCommissionMilestone.targetRevenue - currentTotalRevenue); //
    } else {
            ...
            <div className="bg-white rounded-2xl shadow-lg p-6 border border-gray-100"> {/* */}
                <div className="flex items-center justify-between mb-3"> {/* */}
                    <h3 className="text-lg font-bold text-gray-900 flex items-center gap-2"> {/* */}
                       <i className="fas fa-percent text-orange-500"></i> Commission Milestone Progress {/* */}
                    </h3>
                    <span className="text-sm font-semibold text-gray-700"> {/* */}
                       Current Rate: <span className="text-orange-600 font-black">{currentCommissionRate}%</span> {/* */}
                    </span>
                </div>
                ...
                {nextCommissionMilestone ? ( //
                    <>
                        ...
                        <div className="flex justify-between text-sm text-gray-600"> {/* */}
                           <span>{formatCurrency(currentTotalRevenue)} Revenue</span> {/* */}
                            <span>{formatCurrency(revenueToNextMilestone)} to next rate ({nextCommissionMilestone.commissionRate}%)</span> {/* */}
                        </div>
                        {commissionProgress > 0 && commissionProgress < 100 && ( //
                            <div className="text-center mt-2 text-xs font-medium text-gray-500"> {/* */}
                                {commissionProgress.toFixed(1)}% to next milestone ({formatCurrency(nextCommissionMilestone.targetRevenue)}) {/* */}
                            </div>
                        )}
                    </>
                ) : (
                    ...
                    <>
                         ...
                         <div className="text-center mt-2 text-sm font-semibold text-green-700"> {/* */}
                             🎉 You've reached the final commission rate of {currentCommissionRate}%! {/* */}
                         </div>
              ...
              {/* Responsive Milestones Grid */}
               {allDisplayMilestones.length > 0 ? ( // Check if there are milestones to display
                 <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6"> {/* */}
                    {allDisplayMilestones.map((milestone) => ( //
                    <div
                        key={milestone.id} //
