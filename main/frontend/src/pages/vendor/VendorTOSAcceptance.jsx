...
// Content extracted from user-provided TOS
const tosContent = [
    {
    ...
    },
    {
        ...
        subSection: {
            title: "While messaging/ communicating with Users and or Vendors you shall not:",
            list: [
...
const VendorTOSAcceptance = () => {
  const navigate = useNavigate();
  const currentUser = useAuthStore((s) => s.currentUser);
  const acceptTOS = useAuthStore((s) => s.acceptTOS);

  // Redirect if not a vendor or if TOS is already accepted
  ...
  const handleAccept = () => {
    // Note: In a real app, you would call an API here to persist acceptance.
    // For this simulation, we update the local store state.
    acceptTOS("vendor");
    // Navigate immediately to the protected area
  ...
  return (
    <div className="flex flex-col min-h-screen bg-gray-50">
      <Navbar />
      <main className="flex-grow container mx-auto px-4 py-8 pt-24 max-w-5xl">
        <div className="bg-white rounded-2xl shadow-xl border border-gray-100 p-6 md:p-8 lg:p-10">
          <div className="text-center mb-8">
            ...
            <p className="text-gray-600">
              You must accept the terms below to continue to the Vendor Dashboard.
            </p>
          </div>
                    ...
                    <div key={sectionIndex} className="space-y-3">
                        ...
                        {section.subSection && (
                            <div className="mt-4 p-3 bg-white rounded-lg border border-gray-200">
                                <h3 className="font-semibold text-gray-800 mb-2">{section.subSection.title}</h3>
                                {section.subSection.text && <p className="text-gray-700 leading-relaxed">{section.subSection.text}</p>}
