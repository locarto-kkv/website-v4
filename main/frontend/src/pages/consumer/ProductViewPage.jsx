    ...
    >
        ...
        <div className="p-4">
            ...
            <div className="flex items-center justify-between">
                <p className="text-xl font-extrabold text-orange-600">{formatCurrency(product.price)}</p>
                <div className="flex items-center gap-1 text-yellow-400">
                    <i className="fas fa-star text-xs"></i>
                    <span className="text-xs text-gray-600 font-semibold">{product.avg_review || 0}</span>
                </div>
            </div>
        </div>
    </Link>
...
const ProductViewPage = () => {
        ...
        const findProduct = () => {
            ...
            for (const blog of blogs) {
                if (blog.products && blog.products.length > 0) {
                    foundProduct = blog.products.find(p => String(p.id) === String(productId));
    ...
    useEffect(() => {
        if (currentUser?.type === 'consumer' && consumerDataContext?.lists && product) {
            setIsInWishlist(consumerDataContext.lists.wishlist?.some(item => item.product_id === product.id) || false);
            const foundCartItem = consumerDataContext.lists.cart?.find(item => item.id === product.id);
        ...
        return (
            <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-gray-50 to-orange-50">
                <div className="text-center">
                    ...
                    <p className="text-gray-600 font-semibold">Loading amazing product...</p>
                </div>
            </div>
        );
    }
        ...
        <div className="flex flex-col min-h-screen bg-gradient-to-br from-gray-50 via-orange-50/30 to-gray-50">
            ...
            <main className="flex-grow container mx-auto px-4 py-8 pt-24 max-w-7xl">
                ...
                <nav className="mb-6 flex items-center gap-2 text-sm">
                    <Link to="/map" className="text-gray-500 hover:text-orange-600 transition">
                        Home
                    </Link>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 md:gap-12 mb-16">
                    ...
                    <div className="space-y-4">
                        <div className="relative aspect-square bg-white rounded-3xl overflow-hidden shadow-2xl border-2 border-gray-100 group">
                            ...
                            >
                                <i className={`fas fa-heart text-2xl ${isInWishlist ? 'animate-pulse' : ''}`}></i>
                            </button>
                        ...
                        <div className="grid grid-cols-3 gap-4 pt-4">
                            {[
                                { icon: 'fas fa-shield-check', text: 'Authentic', color: 'from-green-500 to-emerald-500' },
                                { icon: 'fas fa-truck-fast', text: 'Fast Delivery', color: 'from-blue-500 to-cyan-500' },
                                { icon: 'fas fa-undo', text: 'Easy Returns', color: 'from-purple-500 to-pink-500' },
                            ].map((feature, idx) => (
                                <div key={idx} className={`bg-gradient-to-br ${feature.color} rounded-2xl p-4 text-white text-center shadow-lg`}>
                            ...
                            <div className="flex items-center gap-4 mb-6 pb-6 border-b border-gray-200">
                                <div className="flex items-center gap-2 bg-gradient-to-r from-yellow-50 to-orange-50 px-4 py-2 rounded-xl border border-yellow-200">
                                    <div className="flex text-yellow-400">
                                        {[...Array(5)].map((_, i) => (
                                            <i key={i} className={`fas fa-star text-sm ${i < Math.round(product.avg_review || 0) ? '' : 'text-gray-300'}`}></i>
                                        ))}
                                    </div>
                        ...
                        {product.colors && product.colors.length > 0 && (
                            <div className="bg-white rounded-3xl shadow-xl border border-gray-100 p-6">
                                <h3 className="text-lg font-bold text-gray-800 mb-4 flex items-center gap-2">
                                    ...
                                    Select Color: <span className="text-orange-600 capitalize">{selectedColor}</span>
                                </h3>
                            ...
                            <div>
                                ...
                                <div className="flex items-center gap-3">
                                    <div className="flex items-center bg-gray-100 rounded-xl overflow-hidden border-2 border-gray-200">
                                        ...
                                        <span className="w-16 text-center font-bold text-2xl text-gray-800">{quantity}</span>
                                        <button
                                            onClick={() => handleQuantityChange(1)}
                                            className="w-14 h-14 flex items-center justify-center text-gray-600 hover:bg-orange-500 hover:text-white transition-all"
                                        >
                                    ...
                                    {cartItem && (
                                        <div className="flex-1 bg-blue-50 border-2 border-blue-200 rounded-xl px-4 py-2 text-center">
                                            <p className="text-xs text-blue-600 font-semibold">In Cart</p>
                                            <p className="text-sm font-bold text-blue-700">{cartItem.quantity} items</p>
                                        </div>
                                    )}
                                </div>
                            </div>
                    ...
                    <div className="mt-16 pt-12 border-t-2 border-gray-200">
                        <div className="flex items-center justify-between mb-8">
                            <div>
                                ...
                                <p className="text-gray-600">Similar products handpicked for you</p>
                            </div>
                            <Link 
                                to="/map" 
