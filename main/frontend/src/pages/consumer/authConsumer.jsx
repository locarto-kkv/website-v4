...
const OtpInput = ({ value, onChange }) => ( // [cite: src/pages/consumer/authConsumer.jsx]
  <div>
    <label
      htmlFor="otp"
...
const PasswordInput = ({ value, onChange, showPassword, setShowPassword }) => ( // [cite: src/pages/consumer/authConsumer.jsx]
  <div>
    <label
      htmlFor="password"
...
const AuthConsumer = () => {
  ...
  const [formData, setFormData] = useState({ // [cite: src/pages/consumer/authConsumer.jsx]
    email: "",
    otp: "",
    password: "",
  ...
  const getButtonText = () => { // [cite: src/pages/consumer/authConsumer.jsx]
     if (usePassword) return loginLoading ? "Logging in..." : "Login with Password"; // [cite: src/pages/consumer/authConsumer.jsx]
    if (!usePassword) { // [cite: src/pages/consumer/authConsumer.jsx]
  ...
  const handleAuthType = () => { // [cite: src/pages/consumer/authConsumer.jsx]
  ...
  const handleChange = (e) => { // [cite: src/pages/consumer/authConsumer.jsx]
    setFormData({ ...formData, [e.target.name]: e.target.value }); // [cite: src/pages/consumer/authConsumer.jsx]
  };

  const handleSubmit = async (e) => { // [cite: src/pages/consumer/authConsumer.jsx]
    e.preventDefault(); // [cite: src/pages/consumer/authConsumer.jsx]

    if (sentOtp || usePassword) { // [cite: src/pages/consumer/authConsumer.jsx]
  ...
  const handleGoogleSubmit = async () => { // Removed 'e' // [cite: src/pages/consumer/authConsumer.jsx]
  ...
  const resendOtp = async () => { // [cite: src/pages/consumer/authConsumer.jsx]
   ...
   useEffect(() => { // [cite: src/pages/consumer/authConsumer.jsx]
    if (cooldown === 0) { // [cite: src/pages/consumer/authConsumer.jsx]
      return;
    }
    const interval = setInterval(() => { // [cite: src/pages/consumer/authConsumer.jsx]
      useAuthStore.setState((state) => { // [cite: src/pages/consumer/authConsumer.jsx]
        if (state.cooldown <= 1) { // [cite: src/pages/consumer/authConsumer.jsx]
      ...
      <div className="bg-white py-8 px-6 shadow-xl rounded-lg sm:px-10 border border-gray-200">
        ...
        <div className="text-center mb-6">
          <h1 className="text-2xl font-bold text-gray-800">
            {isLogin ? "Customer Login" : "Customer Sign Up"} {/* [cite: src/pages/consumer/authConsumer.jsx] */}
          </h1>
           <p className="text-sm text-gray-500 mt-1">Explore local brands and products</p>
        </div>
        ...
        <div>
          ...
          >
            ...
            {isLogin ? "Login with Google" : "Sign up with Google"} {/* [cite: src/pages/consumer/authConsumer.jsx] */}
          </button>
        </div>
