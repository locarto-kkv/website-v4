...
const EditableInput = ({ label, id, name, value, onChange, isEditing, type = "text", placeholder = "" }) => (
    <div>
...
const CustomerSettings = () => {
    ...
    const displayAddress = `${profileData.addressLine1}${profileData.addressLine2 ? ', ' + profileData.addressLine2 : ''}, ${profileData.city || ''}, ${profileData.state || ''} ${profileData.pincode || ''}`.replace(/ ,|, $/g, ''); // Basic formatting

    if (loading && !profileData.name) { // Show loading only on initial load
        return (
             <div className="flex items-center justify-center h-[calc(100vh-150px)]"> {/* Centered loading */}
    ...
    return (
        <div className="grid grid-cols-1 xl:grid-cols-4 gap-6"> {/* Main Grid Layout */}

            {/* Main Settings Content Area */}
            <div className="xl:col-span-3">
                <form onSubmit={handleSaveChanges} className="space-y-6"> {/* Form wraps sections */}
                        ...
                        <div className="space-y-4">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                ...
                                <EditableInput
                                    ...
                                    isEditing={isEditingAccountInfo} type="email"
                                />
                                <EditableInput
                                    label="Phone" id="phone" name="phone"
                                    value={profileData.phone} onChange={handleInputChange}
                                    isEditing={isEditingAccountInfo} type="tel"
                                />
                            </div>
                            <h3 className="text-md font-semibold text-gray-800 pt-4 border-t mt-4">Shipping Address</h3>
                             <EditableInput
                                label="Address Line 1" id="addressLine1" name="addressLine1"
                                value={profileData.addressLine1} onChange={handleInputChange}
                                isEditing={isEditingAccountInfo}
                            />
                            <EditableInput
                                label="Address Line 2 (Optional)" id="addressLine2" name="addressLine2"
                                value={profileData.addressLine2} onChange={handleInputChange}
                                isEditing={isEditingAccountInfo}
                            />
                            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                               <EditableInput
                                    label="City" id="city" name="city"
                                    value={profileData.city} onChange={handleInputChange}
                                    isEditing={isEditingAccountInfo}
                                />
                                <EditableInput
                                    label="State" id="state" name="state"
                                    value={profileData.state} onChange={handleInputChange}
                                    isEditing={isEditingAccountInfo}
                                />
                                <EditableInput
                                    label="Pincode" id="pincode" name="pincode"
                                    value={profileData.pincode} onChange={handleInputChange}
                                    isEditing={isEditingAccountInfo}
                                />
                            </div>
                    ...
                    <div className="bg-white rounded-2xl shadow-lg p-6 border border-gray-100 flex justify-end">
                        <button
                             ...
                             className={`bg-gradient-to-r from-orange-500 to-red-500 text-white py-3 px-8 rounded-xl font-bold hover:shadow-lg transition-all flex items-center gap-2 ${
                                 (loading || !isEditingAccountInfo) ? 'opacity-50 cursor-not-allowed' : '' // Style when disabled
                             }`}
                        >
                            {loading ? (
                                <>
                ...
                {/* Profile Summary Card */}
                 <div className="bg-white rounded-2xl shadow-lg border border-gray-200 p-6">
                    <div className="flex items-center gap-4 mb-4">
                        ...
                        <div>
                             <h3 className="font-bold text-lg text-gray-900">{profileData.name || 'User'}</h3>
                             <p className="text-sm text-gray-500">{profileData.email || 'No email'}</p>
                        </div>
                    </div>
                    <div>
                        <h4 className="font-semibold text-gray-800 mb-2 text-sm">Address:</h4>
                ...
                <div className="bg-white rounded-2xl shadow-lg border border-gray-200 p-6">
                    ...
                    <div className="space-y-3">
                        ...
                        <div className="p-3 bg-gradient-to-br from-yellow-50 to-amber-50 rounded-xl border border-yellow-100">
                            <p className="text-sm text-gray-600 font-medium">Loyalty Points Earned</p>
                ...
                {/* Quick Links Card */}
                 <div className="bg-white rounded-2xl shadow-lg border border-gray-200 p-6">
                    ...
                    <div className="space-y-2">
                         <button onClick={() => navigate('/consumer/dashboard/orders')} className="w-full text-left p-3 rounded-lg hover:bg-gray-100 text-gray-700 font-medium flex items-center gap-3 transition-colors">
                             <i className="fas fa-box w-4 text-center text-gray-500"></i> My Orders
                         </button>
                         <button onClick={() => navigate('/consumer/dashboard/lists')} className="w-full text-left p-3 rounded-lg hover:bg-gray-100 text-gray-700 font-medium flex items-center gap-3 transition-colors">
                            <i className="fas fa-list w-4 text-center text-gray-500"></i> My Lists
                         </button>
                         <button onClick={() => navigate('/consumer/dashboard/support')} className="w-full text-left p-3 rounded-lg hover:bg-gray-100 text-gray-700 font-medium flex items-center gap-3 transition-colors">
