...
const CustomerSupport = () => {
    ...
    const [expandedFAQ, setExpandedFAQ] = useState(null); // State to track which FAQ is open

    // --- FAQ Data (Consider moving to a separate file or fetching) ---
    const faqItems = [
        { id: 1, question: "How to track my order?", answer: "You can track your order status directly from the 'My Orders' page in your dashboard. Click on the order you want to track and select 'Track Order'." },
        { id: 2, question: "What is the return policy?", answer: "Most items can be returned within 14 days of delivery. Please check the specific product page for detailed return eligibility and instructions. Initiate returns from the 'My Orders' page." },
        { id: 3, question: "How to change my address?", answer: "You can update your shipping address in the 'Account Settings' section of your dashboard. Please note, address changes cannot be applied to orders already shipped." },
        { id: 4, question: "What payment methods are accepted?", answer: "We accept major credit cards (Visa, MasterCard, Amex), debit cards, UPI, and Net Banking. Available options may vary." },
        { id: 5, question: "How long does shipping take?", answer: "Shipping times vary depending on the vendor and your location. Estimated delivery times are shown during checkout and on the 'My Orders' page after purchase." },
    ];
    // --- End FAQ Data ---
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div className="bg-white rounded-2xl shadow-lg p-6 border border-gray-100">
                ...
                <div className="space-y-3">
                    {faqItems.map((faq) => (
                        <div key={faq.id} className="border border-gray-200 rounded-xl overflow-hidden transition-all duration-300 hover:border-gray-300"> {/* Added hover border */}
                            <button
                            ...
                            >
                                <div className="flex items-center gap-3">
                                    ...
                                    <span className="flex-1 text-gray-800 font-semibold">{faq.question}</span> {/* Changed font-medium to font-semibold */}
                                </div>
                                <i className={`fas fa-chevron-${expandedFAQ === faq.id ? 'up' : 'down'} text-gray-400 text-sm transition-transform duration-300 ml-2`}></i> {/* Adjusted icon and margin */}
                            </button>

                            {/* Conditionally Rendered Answer */}
                            <div className={`overflow-hidden transition-all duration-300 ease-in-out ${
                                expandedFAQ === faq.id ? 'max-h-96 opacity-100' : 'max-h-0 opacity-0' // Animation classes
                            }`}>
                                <div className="px-4 pb-4 bg-gray-50 border-t border-gray-200"> {/* Style for answer area */}
                                    <p className="text-gray-700 leading-relaxed pt-3 pl-11"> {/* Padding to align with question */}
                                        {faq.answer || "Answer not available yet."} {/* Added fallback */}
                                    </p>
                                </div>
                            </div>
                        </div>
