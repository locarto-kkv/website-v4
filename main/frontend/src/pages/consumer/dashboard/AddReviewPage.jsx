...
const AddReviewPage = () => {
  ...
  const { orders } = useConsumerData(); // Get orders from context
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await addReview(order.product_id, { // Pass productId from the order
        rating: rating,
        comment: comment,
  ...
  if (loading || !order) {
    return (
      <div className="flex justify-center items-center h-64">
        {loading ? "Loading order details..." : "Order not found or cannot be reviewed."}
      </div>
    );
  }
  ...
  return (
    <div className="bg-white rounded-2xl shadow-lg p-4 md:p-6 lg:p-8 border border-gray-100 max-w-2xl mx-auto">
      <div className="flex justify-between items-center mb-4 md:mb-6 pb-4 border-b">
         <h2 className="text-xl md:text-2xl font-bold text-gray-900">Write a Review</h2>
       ...
       <div className="flex items-center gap-4 mb-6 p-4 bg-gray-50 rounded-xl border">
          <img
            src={order.product?.product_images?.[0]?.url || "https://placehold.co/80x80/e2e8f0/94a3b8?text=IMG"}
            alt={order.product?.name || "Product"}
            className="w-16 h-16 md:w-20 md:h-20 object-cover rounded-lg flex-shrink-0 border"
          />
          <div className="flex-1">
             <h3 className="font-bold text-gray-800 text-sm md:text-base">{order.product?.name || "Product Name"}</h3>
             <p className="text-xs text-gray-500">Order #ORD{order.id}</p>
             <p className="text-sm font-semibold text-orange-600 mt-1">{formatCurrency(order.amount)}</p>
          </div>
       </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-5">
        {/* Rating Section */}
        <div>
          <label className="block text-base font-semibold text-gray-800 mb-3">Your Rating *</label>
