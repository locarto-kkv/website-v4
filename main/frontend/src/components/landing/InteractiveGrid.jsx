  ...
  return (
    <div className="w-full relative py-12 sm:py-20 px-2 sm:px-4">
      ...
      <div className="text-center mb-8 sm:mb-16 relative z-10">
        <div className={`relative inline-flex items-center gap-2 sm:gap-3 ${header.badgeBg} px-4 sm:px-6 py-2 rounded-full text-xs sm:text-sm font-bold ${header.badgeColor} shadow-lg`}>
            <span className={`w-2 h-2 ${header.badgeColor.replace('text-', 'bg-')} rounded-full`} />
            {header.badgeText}
          </div>
        
        
        <h2 className="text-2xl sm:text-4xl md:text-5xl font-black mb-4 sm:mb-6 mt-4">
          <span className={`bg-gradient-to-r ${header.gradient} bg-clip-text text-transparent`}>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 sm:gap-6 max-w-7xl mx-auto">
        ...
        {data.slice(0, displayLimit).map((item, index) => (
          ...
          >
            <CardComponent {...(type === "vendor" ? { vendor: item, isMobile } : { product: item, isMobile })} />
          </div>
        ))}
      </div>
      ...
      {/* View All Button */}
       <div className="text-center mt-8 sm:mt-12">
        ...
        >
          ...
          <span>{type === "vendor" ? "View All Brands" : "Explore Products"}</span>
        </Link>
      </div>
