...
import { useConsumerData } from "../context/consumer/consumerDataContext.jsx"; // Added .jsx
...
const DashboardNavbar = ({ onMenuClick }) => {
  ...
  // Safely get consumer data only if it's a consumer
  ...
  const profileLink = () => {
    switch (currentUser?.type) {
      ...
      case "consumer":
        return "/consumer/dashboard/settings"; // Settings page often serves as profile for consumers
      case "admin":
        return "/admin/dashboard"; // Admin might not have a dedicated profile page shown here
      default:
  ...
  // Navigate to lists page when cart icon is clicked (for consumers)
  const handleCartClick = () => {
    if (currentUser?.type === "consumer") {
      navigate("/consumer/dashboard/lists"); // Navigate to the lists page
    }
    // No action needed for other user types as the icon isn't shown
  };
  ...
  return (
    <nav className="bg-white shadow-md h-[70px] px-3 sm:px-4 md:px-6 flex items-center justify-between fixed top-0 left-0 w-full z-50 border-b border-gray-100">
      ...
      <div className="absolute left-1/2 transform -translate-x-1/2 lg:static lg:transform-none lg:translate-x-0 lg:flex-1 lg:ml-4 flex items-center justify-center lg:justify-start">
        ...
        <button
          onClick={onMenuClick} // Toggles the main sidebar on mobile
      ...
      <div className="flex items-center space-x-2 sm:space-x-3 md:space-x-4 flex-shrink-0">
        {/* Customer-only cart - Navigates to lists page */}
        {isConsumer && (
          <button
            onClick={handleCartClick} // <-- Navigates to lists page
        ...
        >
          ...
          {/* <span className="absolute -top-1 -right-1 bg-red-500 text-white text-xs rounded-full h-5 w-5 flex items-center justify-center font-bold shadow-lg">2</span> */}
        </button>
        ...
        <div className="relative flex-shrink-0">
          <button
            ...
            className="flex items-center space-x-2 bg-gray-100 text-gray-700 px-2 sm:px-3 py-2 rounded-lg hover:bg-gray-200 transition ring-1 ring-gray-200 hover:ring-orange-300" // Added ring effect
          ...
          {dropdownOpen && (
            ...
            >
              ...
              <Link
                to={profileLink()}
                className="block w-full text-left px-4 py-3 text-sm text-gray-700 hover:bg-gray-100 transition-colors flex items-center gap-2" // Added flex
                onClick={() => setDropdownOpen(false)}
                role="menuitem"
              >
                <i
                  className={`fas ${
                    currentUser?.type === "consumer" ? "fa-cog" : "fa-user" // Different icons based on role
              ...
              <button
                ...
                className="w-full text-left px-4 py-3 text-sm text-gray-700 hover:bg-gray-100 transition-colors flex items-center gap-2" // Added flex
