...
const DashboardNavbar = ({ onMenuClick }) => {
  ...
  return (
    <nav className="bg-white shadow-md h-[70px] px-3 sm:px-4 md:px-6 flex items-center justify-between fixed top-0 left-0 w-full z-50 border-b border-gray-100"> {/* Use justify-between */}
      {/* Logo - Left most */}
      <div className="flex items-center flex-shrink-0"> {/* Ensure logo doesn't shrink */}
      ...
      <div className="absolute left-1/2 transform -translate-x-1/2 lg:static lg:transform-none lg:translate-x-0 lg:flex-1 lg:ml-4 flex items-center justify-center lg:justify-start"> {/* Center hamburger absolutely on mobile, reset for large screens */}
      ...
      {/* Right side: Actions */}
      <div className="flex items-center space-x-2 sm:space-x-3 md:space-x-4 flex-shrink-0"> {/* Ensure right side doesn't shrink */}
        {/* Customer-only cart */}
        ...
        {/* Notifications */}
        <button className="relative p-2 hover:bg-gray-100 rounded-lg transition-colors flex-shrink-0" aria-label="View Notifications">
        ...
        <div className="relative flex-shrink-0">
          ...
          {dropdownOpen && (
            ...
            >
               {(currentUser?.type === "vendor" || currentUser?.type === "consumer") && (
                 <Link
                   to={profileLink()}
                   ...
                   role="menuitem"
                 >
                   <i className={`fas ${currentUser.type === 'consumer' ? 'fa-cog' : 'fa-user'} mr-2 text-gray-500`}></i>
